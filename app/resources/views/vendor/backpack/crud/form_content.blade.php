    ...
    <script>
        ...
        jQuery('document').ready(function($){
            ...
            function removeErrorMessage(fieldName) {
                // Encuentra el campo por su nombre
                var field = $('input[name="' + fieldName + '"], select[name="' + fieldName + '"], textarea[name="' + fieldName + '"]');
                // Encuentra el contenedor .form-group más cercano y elimina la clase text-danger
                var formGroup = field.closest('.form-group');
                formGroup.removeClass('text-danger');
                // Elimina el mensaje de error específico del campo y quita la clase is-invalid del campo
                formGroup.find('.invalid-feedback').remove();
                field.removeClass('is-invalid');
            }

            function removeTopErrorMessage(fieldName) {
                var parts = fieldName.split('_');
                parts.shift();
                var formattedFieldName = parts.join(' ');
                $('.alert-danger li').each(function() {
                    if ($(this).text().toLowerCase().includes(formattedFieldName)) {
                        $(this).remove();
                    }
                });
                if ($('.alert-danger li').length === 0) {
                    $('.alert-danger').hide();
                }
            }
            ...
            $(document).ready(function() {
                $('input, select, textarea').on('change input', function() {
                    ...
                    if (fieldName && fieldName !== undefined && $(this).val() !== '') {
                        removeErrorMessage(fieldName);
                        removeTopErrorMessage(fieldName);
                    }
                });
            });
        });
