...
class DatasetCrudController extends CrudController
{
    ...
    {
        $diccionario = Dictionary::all(['data_type', 'description', 'default_unit']);
        $diccionarioData = $diccionario->keyBy('data_type')->toArray();
    ...
    public function store(DatasetRequest $request)
    {
        try {
            ...
            $requestDataToMod = json_decode($request_array['engine_template'], true);

            $diccionario = Dictionary::orderBy('id')->pluck('data_type')->toArray();
            $diccionarioData = array_values($diccionario);

            foreach ($requestDataToMod as $key => &$value) {
                $dataTypeIndexToMod = intval($value['data_type']);
                if (isset($diccionarioData[$dataTypeIndexToMod])) {
                    $value['data_type'] = $diccionarioData[$dataTypeIndexToMod];
                }
            }

            $request_array['engine_template'] = json_encode($requestDataToMod);

            $engine_id = Engine::createEngineFromCrudController($request_array['engine_template']);
            if (!$engine_id) {
                Log::error("Error creando el engine");
                Alert::error("No se ha creado correctamente el engine.")->flash();
            ...
            if ($engine_id && $isDatasetCreated) {
                ...
                Alert::error("No se ha creado correctamente el dataset.");
                return redirect()->back()->withInput();
            }
        } catch (\Exception $e) {
